			...
			switch (args.length) {
				case 1:
					if (player.isConsole()) {
						...
						api.sendMessage(player, null, plugin.name + " &cNo se puede ejecutar este comando desde consola&f.", "es");
						return false;
					}
					...
					api.sendMessage(player, null, plugin.name + " &7Su idioma ha sido &cestablecido &7a " + lang + "&f.", "es");
					return true;
				...
				case 2:
					...
					try {
						...
						api.sendMessage(player, null, "&7El jugador &f'&b" + args[0] + "&f' &cno existe&f.", "es");
						player2 = null;
						return false;
					}
					...
					if (!sender.hasPermission("ChatTranslator.lang.others")) {
						api.sendMessage(player, null, "&cUsted no tiene permisos para ejecutar este comando&f.", "es");
						return false;
					}
					...
					api.broadcast(player,
						plugin.name + " &f'&b%player%&f' &7ha cambiado el idioma de &f'&b%player2%&f' &7a &b%lang%&f.".replace("%player2%", player2.getName()).replace("%lang%", lang), null,
						"es"
					);
					return true;
				...
				default:
					...
					api.sendMessage(player, null, plugin.name + " &cSintaxis invalida&f. &aPor favor use la sintaxis&f: &e/lang &f[&6codigo&f]&f.", "es");
					return false;
			}

		} catch (IllegalArgumentException e) {
			player.sendMessage(api.formatMsg(player, null, plugin.name + " " + e.getMessage(), "es", api.getLang(player)));
			return false;
		}
	}
}